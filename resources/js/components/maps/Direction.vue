<template>
  <div class="m-2 mb-4" style="width: 100%; height: 50vh;">
    <MglMap
  ...
  methods: {
    onload(event){
      const map = event.map;
        ...
        directions.setDestination(this.outlet.lng_lat.split(","));

        var element = document.getElementsByClassName("mapbox-directions-instructions-wrapper");
        var i;

        for (i = 0; i < element.length; i++) {
          element[i].classList.add("add-directions-instructions-none");
        }
    }
  },
}
