            ...
            <div class="wrap-table-kelola">
                <div class="mb-4">
                    <div class="d-flex justify-content-between align-items-center mb-5">
                        <p class="mb-0 font-weight-bold" style="color: darkslateblue">Pegawai</p>
                                        ...
                                        <div class="d-flex justify-content-start align-items-center" style="width: 85%">
                                            <div class="mr-3">
                                                <div class="rounded-circle background-primary d-flex justify-content-center align-items-center" style="width: 40px; height: 40px">
                                                    <p class="mb-0 text-white">{{item.role == 'admin' ? 'A' : 'K'}}</p>
                                                </div>
                                            </div>
                                            <div>
                                                <div class="d-flex justify-content-between align-items-center">
                                                    <div class="d-flex flex-wrap justify-content-start align-items-center">
                                                        ...
                                                        <div style="width: 150px">
                                                            <p class="mb-0" style="font-size: 9pt; color: black; font-family: sans-serif">{{item.username}}</p>
                                                        </div>
                                                        <div style="width: 150px">
                                                            <p class="mb-0" style="font-size: 9pt; color: black; font-family: sans-serif">{{item.role}}</p>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                    ...
                    <div class="d-flex justify-content-between align-items-center py-2 px-4 my-3" style="width: 100%; border-radius: 15px; background: rgb(240,243,248)">
                        <div class="d-flex justify-content-start align-items-center">
                            <div class="mr-3">
                                <template>
                                    <DoughnutChart
                                    :percent="lengthAndPercent.admin.persentase"
                                    :visibleValue="true"
                                    :width="80"
                                    :height="80"
                            ...
                            <div>
                                <p class="mb-0 font-weight-bold color-primary" style="font-family: berlin sans fb; font-size: 13pt">Admin</p>
                            </div>
                        </div>
                        <div>
                            <p class="mb-0" style="font-size: 10pt; color: grey">{{lengthAndPercent.admin.jumlah}} Orang</p>
                        </div>
                    </div>
                    <div class="d-flex justify-content-between align-items-center py-2 px-4 my-3" style="width: 100%; border-radius: 15px; background: rgb(240,243,248)">
                        <div class="d-flex justify-content-start align-items-center">
                            ...
                            <div>
                                <p class="mb-0 font-weight-bold color-primary" style="font-family: berlin sans fb; font-size: 13pt">Kasir</p>
                            </div>
                        </div>
                        <div>
...
import Update from "../../components/pegawai/Update";
...
export default {
    ...
    computed: {
        ...
        data(){
            return this.$store.getters['pegawai/getPegawai']
        },
        ...mapGetters({
            getLengthAndPercent : 'pegawai/getLengthAndPercent'
        })
    },
    watch: {
        data(){
    ...
    methods: {
        ...
        async filterFunction(){
            const search = this.pre_items.filter(i => {
                ...
                i.username.toLowerCase().indexOf(this.search.toLowerCase()) >= 0 ||
                i.role.toLowerCase().indexOf(this.search.toLowerCase()) >= 0)
            })
            this.items = search
        },
        redirectCreate(){
        ...
        async redirectEdit(id){
            try{
                await this.$store.dispatch('pegawai/GET_INFO_BY_ID', id)
        ...
        async softDelete(item){
            try{
                await this.$store.dispatch('pegawai/SOFTDELETE_ONE', item)
        ...
        async exportPdf(){
            try {
                ...
                doc.setFontSize(11);
                doc.text('Daftar Pegawai Laundry', 40, 120);
                ...
                doc.save('daftar-pegawai-laundry-' + this.outlet.nama + '.pdf');
    ...
    async created(){
        try {
            let id = this.$route.params.id;
            await this.$store.dispatch('pegawai/GET_ALL', id)
        } catch (error) {
            alert(error);
        }
    }
